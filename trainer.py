...
from utils.dataset_Mos import Mos_dataset, RandomGenerator_Mos, ResizeGenerator_Mos
...
def inference_Mos_val(args, model, best_performance):
    ...
    db_test = Mos_dataset(base_dir=args.volume_path, split="val", list_dir=args.list_dir, nclass=args.num_classes, transform=transforms.Compose([ResizeGenerator_Mos(output_size=[args.img_size, args.img_size])]))
    testloader = DataLoader(db_test, batch_size=1, shuffle=False, num_workers=1)
...
def trainer_mos(args, model, snapshot_path):
    logging.basicConfig(filename=snapshot_path + "/log.txt", level=logging.INFO,
                        format='[%(asctime)s.%(msecs)03d] %(message)s', datefmt='%H:%M:%S')
    logging.getLogger().addHandler(logging.StreamHandler(sys.stdout))
    logging.info(str(args))
    ...
    db_train = Mos_dataset(
        base_dir=args.root_path, 
        list_dir=args.list_dir, 
        split="train", 
        nclass=args.num_classes,
        transform=transforms.Compose([RandomGenerator_Mos(output_size=[args.img_size, args.img_size])])
    )
    ...
    for epoch_num in iterator:
        ...
        performance = inference_Mos_val(args, model, best_performance)
