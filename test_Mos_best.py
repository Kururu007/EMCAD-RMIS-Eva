...
from utils.dataset_Mos import Mos_dataset, ResizeGenerator_Mos
...
parser.add_argument('--volume_path', type=str,
                    default='./data/MosMedPlus_EMCAD/test', help='root dir for validation volume data')
...
def inference_CheX_test(args, model, test_save_path=None):
    ...
    db_test = Mos_dataset(base_dir=args.volume_path, split="test", list_dir=args.list_dir, nclass=args.num_classes, transform=transforms.Compose([ResizeGenerator_Mos(output_size=[args.img_size, args.img_size])]))
    testloader = DataLoader(db_test, batch_size=1, shuffle=False, num_workers=1)
